...
async function authVerify(request, reply, done) {
    ...
    logger.debug('Decoded token header: ' + JSON.stringify(decodedToken.header));
    ...
    jwt.verify(token, signingKey, validateOptions, function (err, decoded) {
        if (!err) {
            ...
            //Validating appid of calling api
            if (__.indexOf(appConfig.quoteApiApprovedCallingApps,decoded.appid,0) < 0) {
                logger.warn('Caller app not valid app caller list');
