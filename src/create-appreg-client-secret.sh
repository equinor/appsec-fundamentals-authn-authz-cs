...
APPID=$(getAppId "$CFG_APPREG_CLIENT_NAME")

if [ ! -n "$APPID" ] && [ "$APPID" -eq 0 ]
then
    printf "Unable to find appId for %s \n, exiting" "$CFG_APPREG_CLIENT_NAME"
    exit 1
fi

printf "Creating or updating the Secret for Appregistration %s with AppId %s \n" "$CFG_APPREG_CLIENT_NAME" "$APPID"

CLIENT_SECRET=$(az ad app credential reset --id "$APPID" --display-name "$CFG_APPREG_CLIENT_SECRET_NAME" --output tsv --query password | tr -d '\r')
...
# Updating the .env file with the new config
printf "About to update .env file (%s%s)\n" "$CFG_ENV_FILE_DIRECTORY" "$CFG_ENV_CLIENT_FILENAME"

printf ":: CLIENT_SECRET\n"
sed -i'.bkup' 's/CLIENT_SECRET=.*/CLIENT_SECRET='\'"$CLIENT_SECRET"\''/' "$CFG_ENV_FILE_DIRECTORY""$CFG_ENV_CLIENT_FILENAME"
