...
function build(opts = {}) {
    ...
    app.get('/login', async function (request, reply) {
        if (__.isEqual(process.env.NODE_ENV, 'test')) {
            await authUtils.authenticateForTest(request, reply);
        } else {
            request.query.state = authUtils.generateRandomKey(16);
    ...
    app.get(
        ...
        async function (request, reply) {
            ...
            const episodes = await gotEpisodes(accessToken);
