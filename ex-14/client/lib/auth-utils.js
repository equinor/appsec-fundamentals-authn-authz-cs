...
async function requestAccessTokenUsingAuthCode(request,reply,authCode) {
    ...
    if (__.isEmpty(request.session.pkceCodes)) {
        ...
        logger.error('No session object in callback auth redirect, aborting authentication.');
        return '';
    }
    ...
    await confidentialClientApplication
        ...
        .then((response) => {
            ...
            logger.debug('Setting homeAccountId into session cookie: ' + request.session.homeAccountId);
...
async function logoutSessionUser(request){
...
async function getTokenSilently (scopes,request) {
    ...
    logger.debug('Requesting tokens silently ' + JSON.stringify(silentRequest));
...
//
// A helper function to do initial authentication for test scenarios
// This functionality must only be used against test tenants with test data!!
//
async function authenticateForTest(request, reply) {

    logger.warn('About to acquire token for test using username/password - ROPC');
    ...
    await confidentialClientApplication
        ...
        .then((response) => {
            logger.warn('Acquired token by ROPC for test');

            //storing homeAccountID in session
            request.session.homeAccountId = response.account.homeAccountId;
            logger.warn(
                'Setting homeAcccountId into session cookie: ' +
                    request.session.homeAccountId
            );
        })
        .catch((error) => {
            logger.warn('Unable to acquire token for test ::' + error);
        });
...
function isAuthenticated(request) {
    ...
    if (!__.isUndefined(request.session.homeAccountId)) {
        logger.debug('Authentication approved - session account exists');

        //We also need to verify that the homeAccountId is supported by an entry in the token cache
        logger.debug(
            msalTokenCache.getAccountByHomeId('Account id: ' + request.session.homeAccountId)
        );

        return true;
    } else {
        logger.debug('Authentication denied - session account does not exists');
        return false;
...
async function getAccountName(homeAccountId) {
    ...
    if ((!__.isUndefined(account)) && (!__.isNull(account))) {
        accountName = {
...
//Helper functiont generate random key
